...
| 内容安全 | `content_safety` | 提供可扩展的 Prompt 与 Schema 骨架 |
| 财务审计 | `financial_audit` | 提供可扩展的 Prompt 与 Schema 骨架 |
| 贷款资质 | `loan_qualification` | 提供可扩展的 Prompt 与 Schema 骨架 |
...
| 业务事件 | 记录节点完成、单次拦截、推理失败和最终拒绝 | `business_events.alert_severity_threshold` |
...
- 除营销文案外，其余预设场景主要用于展示资源切换方式，需要领域维护者继续补充业务规则。
